...
public class CrashUtils {
    static Main plugin;
    public CrashUtils(Main plugin) {
        CrashUtils.plugin = plugin;
    }
    ...
    public static void crashPlayer(CommandSender crasher, Player victim, CrashType crashType) {
        try {
         ...
         //   crasher.sendMessage(plugin.ConsolePrefix + "§aCrashed §2" + victim.getName() + " §ausing §3" + crashType.name() + " §amethod!");

        } catch (Exception e) {

            //crasher.sendMessage(plugin.ConsolePrefix + "§cFailed to crash §e" + victim.getName() + " §eusing " + crashType.name() + " §cmethod!");
